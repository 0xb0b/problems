...
There are two types: Mach bombs (M) and Facula bombs (F). The bombs, once
released into the LAMBCHOP's inner workings, will automatically deploy to all
...
For example, if M = "2" and F = "1", one generation would need to pass, so the
solution would be "1". However, if M = "2" and F = "4", it would not be
possible.
...
def solution(m_str, f_str):
    m_target = int(m_str)
    f_target = int(f_str)
    num_replication_cycles = 0
    x = min(m_target, f_target)
    y = max(m_target, f_target)
    while x > 0:
        if x == 1:
            num_replication_cycles += y - 1
            break
        remainder = y % x
        if remainder == 0:
            num_replication_cycles = 0
            break
        num_replication_cycles += y // x
        y = x
        x = remainder
    if num_replication_cycles:
        return str(num_replication_cycles)
    else:
        return 'impossible'
...
assert solution('1', '4000000000') == '3999999999'
